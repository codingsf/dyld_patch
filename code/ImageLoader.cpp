...
{
    //log("interposedAddress(0x%08llX), tupleCount=%lu\n", (uint64_t)address, fgInterposingTuples.size());
    for (std::vector<InterposeTuple>::iterator it=fgInterposingTuples.begin(); it != fgInterposingTuples.end(); it++) {
        //log("    interposedAddress: replacee=0x%08llX, replacement=0x%08llX, neverImage=%p, onlyImage=%p, inImage=%p\n",
        ...
        if ( (address == it->replacee) && (inImage != it->neverImage) && ((it->onlyImage == NULL) || (inImage == it->onlyImage)) ) {
            if ( context.verboseInterposing ) {
                log("dyld interposing: replace 0x%lX with 0x%lX\n", it->replacee, it->replacement);
            }
...
{
    //log("ImageLoader::link(%s) refCount=%d, neverUnload=%d\n", imagePath, fDlopenReferenceCount, fNeverUnload);
        ...
        for(unsigned int i=0; i < fLibraryCount; ++i){
            ...
            try {
                ...
                if ( dependentLib == this ) {
                    ...
                    if ( dependentLib != this )
                        warn("DYLD_ setting caused circular dependency in %s\n", this->getPath());
                }
                if ( fNeverUnload )
...
void ImageLoader::weakBind(const LinkContext& context)
{
    ...
    if ( (countOfImagesWithWeakDefinitionsNotInSharedCache > 0) && (countNotYetWeakBound > 0) ) {
        ...
        while ( doneCount != count ) {
            //for(int i=0; i < count; ++i)
            //	log("sym[%d]=%s ", sortedIts[i]->loadOrder, sortedIts[i]->symbolName);
            //log("\n");
            ...
            if ( sortedIts[0]->symbolMatches && !sortedIts[0]->done ) {
                ...
                if ( targetAddr != 0 ) {
                    if ( context.verboseWeakBind ) {
                        log("dyld: weak binding all uses of %s to copy from %s\n",
                                  nameToCoalesce, targetImage->getIndexedShortName(targetImageIndex));
                    }
                    for(int i=0; i < count; ++i) {
                        if ( strcmp(iterators[i].symbolName, nameToCoalesce) == 0 ) {
                            if ( context.verboseWeakBind ) {
                                log("dyld: weak bind, setting all uses of %s in %s to 0x%lX from %s\n",
...
void ImageLoader::printStatisticsDetails(unsigned int imageCount, const InitializerTimingList& timingInfo)
{
    ...
    log("  total images loaded:  %d (%u from dyld shared cache)\n", imageCount, fgImagesUsedFromSharedCache);
    log("  total segments mapped: %u, into %llu pages with %llu pages pre-fetched\n", fgTotalSegmentsMapped, fgTotalBytesMapped/4096, fgTotalBytesPreFetched/4096);
    ...
    if ( fgTotalBindSymbolsResolved != 0 ) {
        ...
        log("total binding symbol lookups: %s, average images searched per symbol: %u.%u\n",
...
const uint8_t* ImageLoader::trieWalk(const uint8_t* start, const uint8_t* end, const char* s)
{
    ...
    while ( p != NULL ) {
        ...
        if ( (*s == '\0') && (terminalSize != 0) ) {
            //log("trieWalk(%p) returning %p\n", start, p);
            return p;
        }
        const uint8_t* children = p + terminalSize;
        if ( children > end ) {
            log("trieWalk() malformed trie node, terminalSize=0x%lx extends past end of trie\n", terminalSize);
            return NULL;
        }
        //log("trieWalk(%p) sym=%s, terminalSize=%lu, children=%p\n", start, s, terminalSize, children);
        uint8_t childrenRemaining = *children++;
        p = children;
        uintptr_t nodeOffset = 0;
        for (; childrenRemaining > 0; --childrenRemaining) {
            const char* ss = s;
            //log("trieWalk(%p) child str=%s\n", start, (char*)p);
            ...
            if ( wrongEdge ) {
                ...
                if ( p > end ) {
                    log("trieWalk() malformed trie node, child node extends past end of trie\n");
                    return NULL;
                }
            }
            else {
                ...
                if ( (nodeOffset == 0) || ( &start[nodeOffset] > end) ) {
                    log("trieWalk() malformed trie child, nodeOffset=0x%lx out of range\n", nodeOffset);
                    return NULL;
                }
                s = ss;
                //log("trieWalk() found matching edge advancing to node 0x%lx\n", nodeOffset);
                break;
            }
        }
        if ( nodeOffset != 0 )
