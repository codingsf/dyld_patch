...
#define ARCH_CACHE_MAGIC_H	"dyld_v1 x86_64h"
	...
	if ( ((stat(IPHONE_DYLD_SHARED_CACHE_DIR "enable-dylibs-to-override-cache", &enableStatBuf) == 0)
			&& (enableStatBuf.st_size < ENABLE_DYLIBS_TO_OVERRIDE_CACHE_SIZE)
			&& (stat(IPHONE_DYLD_SHARED_CACHE_DIR DYLD_SHARED_CACHE_BASE_NAME ARCH_NAME DYLD_SHARED_CACHE_DEVELOPMENT_EXT, &devCacheStatBuf) == 0))
		|| (stat(IPHONE_DYLD_SHARED_CACHE_DIR DYLD_SHARED_CACHE_BASE_NAME ARCH_NAME, &prodCacheStatBuf) != 0))
...
static SyscallHelpers sSysCalls = {
		...
		&closedir,
		// added in version 4
//		&SyscallHelpers::coresymbolication_load_notifier,
//		&SyscallHelpers::coresymbolication_unload_notifier,
//		// Added in version 5
//		&proc_regionfilename,
//		&getpid,
...
//
	...
	if ( rootPath != NULL ) {
		...
		int fd = open(simDyldPath, O_RDONLY, 0);
		if ( fd != -1 ) {
