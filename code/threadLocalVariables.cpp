...
extern "C" void* tlv_get_addr(TLVDescriptor* desp_);
...
static void tlv_set_key_for_image(const struct mach_header* mh, pthread_key_t key)
{
    ...
    if ( tlv_live_image_used_count == tlv_live_image_alloc_count ) {
        ...
        struct TLVImageInfo* newBuffer = (TLVImageInfo *)malloc(sizeof(TLVImageInfo)*newCount);
...
{
    TLVHandler *h = (TLVHandler *)malloc(sizeof(TLVHandler));
    h->state = state;
