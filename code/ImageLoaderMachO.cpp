...
extern "C" void warn(char const*, ...);
extern "C" void log(char const*, ...);
extern void throwf(char const*, ...);
extern const char* mkstringf(const char* format, ...);
...
{
    //log("ImageLoader=%ld, ImageLoaderMachO=%ld, ImageLoaderMachOClassic=%ld, ImageLoaderMachOCompressed=%ld\n",
        ...
        for(unsigned int i=0, e=segmentCount(); i < e; ++i) {
            if ( segWriteable(i) && (segFileSize(i) > 0) ) {
                ...
                if ( context.verboseMapping ) {
                    log("%18s prefetching 0x%0lX -> 0x%0lX\n",
    ...
    if ( codeSigCmd == NULL) {
        ...
        if ( result == -1 ) {
            ...
            if ( context.verboseCodeSignatures )
                log("dyld: Failed registering code signature for %s, errno=%d\n", this->getPath(), errno);
            siginfo.fs_file_start = UINT64_MAX;
        } else if ( context.verboseCodeSignatures )  {
            log("dyld: Registered code signature for %s\n", this->getPath());
        }
            ...
            case LC_RPATH:
                ...
                if ( (strncmp(path, "@loader_path", 12) == 0) && ((path[12] == '/') || (path[12] == '\0')) ) {
                    ...
                    if ( context.processIsRestricted && (context.mainExecutable == this) ) {
						warn("LC_RPATH %s in %s being ignored in restricted program because of @loader_path\n", path, this->getPath());
						break;
					}
#endif
                    ...
                    if ( context.processIsRestricted ) {
						warn("LC_RPATH %s in %s being ignored in restricted program because of @executable_path\n", path, this->getPath());
						break;
					}
#endif
                    char resolvedPath[PATH_MAX];
					...
					warn("LC_RPATH %s in %s being ignored in restricted program because it is a relative path\n", path, this->getPath());
					break;
				}
#endif
                    ...
                    for(const char** rp = context.rootPaths; *rp != NULL; ++rp) {
                        ...
                        if ( stat(newPath, &stat_buf) != -1 ) {
                            //log("combined DYLD_ROOT_PATH and LC_RPATH: %s\n", newPath);
...
void ImageLoaderMachO::doRebase(const LinkContext& context)
{
    ...
    if ( context.verbosePrebinding ) {
        if ( !this->isPrebindable() ) {
            log("dyld: image not prebound, so could not use prebinding in %s\n", this->getPath());
        }
        else if ( fSlide != 0 ) {
            log("dyld: image slid, so could not use prebinding in %s\n", this->getPath());
        }
        else if ( !this->allDependentLibrariesAsWhenPreBound() ) {
            log("dyld: dependent libraries changed, so could not use prebinding in %s\n", this->getPath());
        }
        else if ( !this->usesTwoLevelNameSpace() ){
            log("dyld: image uses flat-namespace so, parts of prebinding ignored %s\n", this->getPath());
        }
        else {
            log("dyld: environment variable disabled use of prebinding in %s\n", this->getPath());
        }
    }

    //log("slide=0x%08lX for %s\n", slide, this->getPath());
                                         ...
                                         intptr_t addend, const char* inPath, const char* toPath, const char* msg)
{
    ...
    if ( context.verboseBind ) {
        if ( addend != 0 )
            log("dyld: %sbind: %s:0x%08lX = %s:%s, *0x%08lX = 0x%08lX + %ld\n",
            ...
            log("dyld: %sbind: %s:0x%08lX = %s:%s, *0x%08lX = 0x%08lX\n",
                      msg, shortName(inPath), (uintptr_t)location,
                      ((toPath != NULL) ? shortName(toPath) : "<missing weak_import>"),
                                ...
                                if ( (sect->size > offsetof(DATAdyld, vars)) && (dd->vars.mh == mh) ) {
									...
									if ( memcmp(this->getMain(), sStandardEntryPointInstructions, 16) != 0 ) {
										if ( context.verboseInit )
											log("dyld: program uses non-standard entry point so delaying running of initializers\n");
        ...
        for (uint32_t i = 0; i < cmd_count; ++i) {
            switch (cmd->cmd) {
                case LC_ROUTINES_COMMAND:
                    ...
                    if ( context.verboseInit )
                        log("dyld: calling -init function %p in %s\n", func, this->getPath());
                ...
                for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
                    ...
                    if ( type == S_MOD_INIT_FUNC_POINTERS ) {
                        ...
                        for (size_t j=0; j < count; ++j) {
                            ...
                            if ( context.verboseInit )
                                log("dyld: calling initializer function %p in %s\n", func, this->getPath());
                ...
                for (const struct macho_section* sect=sectionsStart; sect < sectionsEnd; ++sect) {
                    ...
                    if ( type == S_MOD_TERM_FUNC_POINTERS ) {
                        ...
                        for (size_t j=count; j > 0; --j) {
                            ...
                            if ( context.verboseInit )
                                log("dyld: calling termination function %p in %s\n", func, this->getPath());
                            func();
                        }
                    }
                }
...
void ImageLoaderMachO::printStatisticsDetails(unsigned int imageCount, const InitializerTimingList& timingInfo)
{
    ...
    log("total symbol trie searches:    %d\n", fgSymbolTrieSearchs);
    log("total symbol table binary searches:    %d\n", fgSymbolTableBinarySearchs);
    log("total images defining weak symbols:  %u\n", fgImagesHasWeakDefinitions);
    log("total images using weak symbols:  %u\n", fgImagesRequiringCoalescing);
}
...
uintptr_t ImageLoaderMachO::reserveAnAddressRange(size_t length, const ImageLoader::LinkContext& context)
{
    ...
    if ( fgNextPIEDylibAddress != 0 ) {
        ...
        //log("padding 0x%08llX, guard=0x%08llX\n", (long long)(addr - fgNextPIEDylibAddress), (long long)(__stack_chk_guard));
...
void ImageLoaderMachO::segProtect(unsigned int segIndex, const ImageLoader::LinkContext& context)
{
    ...
    if ( context.verboseMapping ) {
        log("%18s at 0x%08lX->0x%08lX altered permissions to %c%c%c\n", segName(segIndex), (uintptr_t)addr, (uintptr_t)addr+size-1,
                  (protection & PROT_READ) ? 'r' : '.',  (protection & PROT_WRITE) ? 'w' : '.',  (protection & PROT_EXEC) ? 'x' : '.' );
    }
}
